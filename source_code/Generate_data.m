 %% Synthetic data
 function Syn_P = Generate_data(number,m, N,q_num, PT,nl)   %N:number of stored patterns, number: test set size of one pattern like H&S, m:scaling size
...
%         x(j) =  round(x(j)-(x(j) - x(j-1))*2.5/3 + ((x(j)+(x(j+1) - x(j))*2.5/3) - (x(j)-(x(j) - x(j-1))*2.5/3))*rand(1));
