...
% data = Generate_data(200,scale,N,q_num, PT,0.2);  %number of warping pattern, scaling, number of template
% data = Generate_negative(200,scale,N,q_num, PT,0.2);
...
%     Template.represent(:,:,i) = Func.Grid8(scaled_pt(i,:),12);
end
...
for i = 1:N
    p = reshape(Template.represent1(:,:,i),[neuron_num, 1]);
    X(i,:) = p;
end
...
%         ind = N;
%     end
    plot(1:m,data(u,:,cc),'LineWidth',1)
...
for u = 1:N
    subplot(3,4,u);
    plot(1:q_num,Template.PT(u,:),'-*','LineWidth',2)
